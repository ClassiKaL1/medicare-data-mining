...
BEGIN
    SET @sql = CONCAT(
        'SELECT 
         ...
         FROM beneficiarysummarysample_2009
...
BEGIN
    SET @sql = CONCAT(
        'SELECT 
            CASE 
                WHEN (2009 - LEFT(birth_date, 4)) < 45 THEN "Under 45"
                WHEN (2009 - LEFT(birth_date, 4)) BETWEEN 45 AND 64 THEN "45-64"
                WHEN (2009 - LEFT(birth_date, 4)) BETWEEN 65 AND 74 THEN "65-74"
                WHEN (2009 - LEFT(birth_date, 4)) BETWEEN 75 AND 84 THEN "75-84"
                ELSE "85+" 
            END AS Age_Group,
            COUNT(*) AS Patient_Count,
            ROUND(AVG(payment_inpatient), 2) AS Avg_Inpatient_Cost,
